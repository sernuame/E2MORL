...
UT = np.zeros(100, dtype=float)
init_UT = -9999
UT += init_UT
temp_buffer = []
evo_exp_enabled_t = 250000
max_used_num = 5
...
def evaluation_and_update(p, c_w, t):
	idx = min(int(c_w[0] * 100), len(UT)-1)
	if np.sum(fitness[p] * c_w) > UT[idx] and UT[idx] != init_UT and used_num[p] < max_used_num:
		# pop_exp -> RL
		if t >= evo_exp_enabled_t:
			for i in range(len(pop_exp[p])):
				exp = pop_exp[p][i]
				exp[0][-2], exp[0][-1] = c_w[0], c_w[1]
				exp[2][-2], exp[2][-1] = c_w[0], c_w[1]
				RL_agent.replay_buffer.add(exp[0], exp[1], exp[2], exp[3], c_w, exp[5])
			used_num[p] += 1
		# try update parent's weight
		p_w = pop_exp_w[p]
		idx1 = min(int(p_w[0]*100), len(UT)-1)
		if np.sum(fitness[p] * c_w) - UT[idx] > np.sum(fitness[p] * p_w) - UT[idx1]:  # if child's weight can bring more improvement
	...
	for t in range(int(args.max_timesteps+1)):
		...
		if ep_finished:
			...
			if t >= args.timesteps_before_training:
				allow_train = True
			idx = min(int(weights[0] * 100), len(UT) - 1)
			if np.sum(ep_total_reward * weights) > UT[idx]:
				UT[idx] = np.sum(ep_total_reward * weights)
			exp_crossover(t)
			exp_mutation(t)
...
if __name__ == "__main__":
	...
	run_name = f"{env_tag}__E2MORL__{seed}__{time_str}"
