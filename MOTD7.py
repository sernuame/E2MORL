...
import buffer_multi_value as buffer
...
class Agent(object):
	def __init__(self, state_dim, action_dim, reward_dim, max_action, offline=False, hp=Hyperparameters()):
		...
		self.max = (torch.zeros([1, reward_dim]) + -1e8).to(self.device)
		self.min = (torch.zeros([1, reward_dim]) + 1e8).to(self.device)
		self.max_target = torch.zeros([1, reward_dim]).to(self.device)
		self.min_target = torch.zeros([1, reward_dim]).to(self.device)
		self.max_abs = (torch.zeros([1, reward_dim])+1).to(self.device)

		self.base_dim = [1]
		self.other_dims = [i for i in range(0, reward_dim)]
	...
	def train(self, f):
		...
		with torch.no_grad():
			...
			Q1_target, Q2_target = self.critic_target(next_state, next_action, fixed_target_zsa, fixed_target_zs)

			mask = Q1_target.abs() <= Q2_target.abs()
			Q_target = torch.where(mask, Q1_target, Q2_target)

			Q_target = reward + not_done * self.hp.discount * torch.maximum(torch.minimum(Q_target, self.max_target), self.min_target)
			self.max = torch.max(self.max, Q_target.max(dim=0)[0])
			self.min = torch.min(self.min, Q_target.min(dim=0)[0])
		...
		if self.training_steps % self.hp.policy_freq == 0:
			...
			with torch.no_grad():
				factor1 = torch.clamp((Q1.abs() / (Q1[:, self.base_dim].abs() + 1e-8)) / torch.clamp(weights/(weights[:, self.base_dim]+1e-8), min=1.0), min=1.0)
				factor2 = torch.clamp((Q2.abs() / (Q2[:, self.base_dim].abs() + 1e-8)) / torch.clamp(weights / (weights[:, self.base_dim] + 1e-8), min=1.0), min=1.0)
				factor1[:, self.other_dims] = 1
				factor2[:, self.other_dims] = 1
			normalized_Q1, normalized_Q2 = Q1/factor1, Q2/factor2

			wQ1, wQ2 = torch.sum(normalized_Q1 * weights, dim=1), torch.sum(normalized_Q2 * weights, dim=1)
		...
		if self.training_steps % self.hp.target_update_rate == 0:
			...
			self.max_target.copy_(self.max)
			self.min_target.copy_(self.min)
			self.max_abs = torch.max(self.max_target.abs(), self.min_target.abs())
